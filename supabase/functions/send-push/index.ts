...
serve(async (req) => {
  ...
  try {
    ...
    if (subError || !subscriptions?.length) {
      edgeLog("info", "send-push skip", { ownerId, reason: subError?.message ?? "no subscriptions" })
      return new Response(
        JSON.stringify({ ok: true, sent: 0, reason: subError?.message ?? "no subscriptions for user" }),
        { headers: { ...buildCorsHeaders(req), "Content-Type": "application/json" } }
      )
    }
