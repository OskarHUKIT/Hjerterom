...
/** Mørk tekst på lys «info»-bakgrunn – overstyrer global `p { color: var(--text-body) }` (lys tekst i mørk modus). */
const GDPR_NOTE_TEXT = '#0f172a'
const GDPR_NOTE_HEADING = '#020617'

/** WCAG / Apple HIG: min. ca. 44×44 px pekeflate for knapper */
const TOUCH_MIN_PX = 44

const attachPhotosButtonStyle: CSSProperties = {
  display: 'inline-flex',
  alignItems: 'center',
...
const removePhotoButtonStyle: CSSProperties = {
  position: 'absolute',
  top: -8,
  right: -8,
  width: TOUCH_MIN_PX,
...
export default function HandoverReport({
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const {
          data: { publicUrl },
        } = supabase.storage.from('handover-reports').getPublicUrl(filePath)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (reporterType === 'tenant') {
      if (!tenantComment.trim() && photoUrls.length === 0) {
        setSubmitError(t('tenantHandoverNeedCommentOrPhoto'))
        return
      }
    } else {
    ...
    try {
      const content =
        reporterType === 'tenant'
          ? { tenant_comment: tenantComment.trim(), photo_urls: photoUrls }
  ...
  const tenantSubmitDisabled =
    loading || submitSuccess || (!tenantComment.trim() && photoUrls.length === 0)
  const homeownerSubmitDisabled =
    loading ||
    !formData.photos_confirmed ||
    (reporterType === 'homeowner' && !submissionConfirmed) ||
    submitSuccess
    ...
    >
      {reporterType !== 'tenant' && (
        <>
          ...
          >
            Ved formidling av bolig gjennom Boly, plikter utleier å dokumentere boligens tilstand.
            Overtakelsesrapporten skal være utfylt og innsendt senest samme dag som utleie starter.
            Boligutleier må legge ved bilder som ikke er eldre enn tre måneder og må være tatt etter
            siste leietaker.
          </p>
        </>
      )}
        ...
        >
          <p
            style={{
              ...
              fontWeight: 700,
              color: GDPR_NOTE_HEADING,
              fontSize: '1rem',
            }}
          >
            Personvern (GDPR)
          </p>
          <p style={{ margin: '0 0 var(--space-3)', color: GDPR_NOTE_TEXT }}>
            {t('tenantHandoverGdprP1')}
          </p>
          <p style={{ margin: 0, color: GDPR_NOTE_TEXT }}>{t('tenantHandoverGdprP2')}</p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: 'grid', gap: 0 }}>
          {reporterType === 'tenant' ? (
            <>
              <section style={SECTION_STYLE} aria-labelledby="hr-tenant-comment">
                {sectionHeader('1', t('tenantHandoverCommentLabel'))}
                <div style={{ padding: '12px 14px', background: 'white' }}>
                  <textarea
                    id="hr-tenant-comment"
                    value={tenantComment}
                    onChange={(e) => setTenantComment(e.target.value)}
                    placeholder={t('tenantHandoverCommentPlaceholder')}
                    rows={5}
              ...
              <section style={SECTION_STYLE}>
                {sectionHeader('2', t('tenantHandoverPhotosSection'))}
                <div style={{ padding: '12px 14px', background: 'white' }}>
                  <input
                    ref={fileInputRef}
                    type="file"
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                    <button
                      ...
                      style={{
                        ...attachPhotosButtonStyle,
                        cursor: uploadingPhotos ? 'wait' : 'pointer',
                        opacity: uploadingPhotos ? 0.85 : 1,
                      }}
                    >
                    ...
                    <span
                      style={{
                        fontSize: '0.95rem',
                        fontWeight: 600,
                        color: photoUrls.length > 0 ? '#0d9488' : '#64748b',
                      }}
                    >
                      {photoUrls.length === 0
                        ? 'Ingen bilder lagt ved ennå'
                        : `${photoUrls.length} bilder lastet opp`}
                    </span>
                  </div>
                  {uploadError && (
                    <p style={{ margin: '8px 0 0', fontSize: '0.9rem', color: '#dc2626' }}>
                      {uploadError}
                    </p>
                  )}
                  {photoUrls.length > 0 && (
                ...
                <div style={{ padding: '12px 14px', background: 'white' }}>
                  ...
                  >
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({ ...formData, photos_confirmed: e.target.checked })
                      }
        ...
        {reporterType === 'homeowner' && (
          ...
          >
            ...
            >
              Jeg ({reporterName || '…'}) bekrefter at jeg sender inn overtakelsesrapport (
              {formatDateNo(formData.photos_date || new Date().toISOString().split('T')[0])}).
            </span>
          </label>
        )}
