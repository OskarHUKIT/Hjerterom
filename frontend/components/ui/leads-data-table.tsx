...
export function LeadsTable({
  ...
  const getSourcePill = (source: string, sourceType: "organic" | "campaign") => {
    ...
    return (
      ...
      >
        {source}
        {!isOrganic ? <span className="ml-1 text-xs opacity-60">↗</span> : null}
      </div>
    );
  };
  ...
  const getStatusPill = (status: Lead["status"]) => {
    ...
    return (
      <div
        className={`rounded-lg border px-2 py-1 text-xs font-medium ${config.bg} ${config.text} ${config.border}`}
      >
        {config.label}
      </div>
    );
  ...
  const getProbabilityIcon = (probability: Lead["probability"]) => {
    ...
    const probabilityColors = {
      low: "border-orange-200 bg-orange-50 text-orange-600 dark:border-orange-800/30 dark:bg-orange-900/20 dark:text-orange-400",
      mid: "border-yellow-200 bg-yellow-50 text-yellow-600 dark:border-yellow-800/30 dark:bg-yellow-900/20 dark:text-yellow-400",
      high: "border-green-200 bg-green-50 text-green-600 dark:border-green-800/30 dark:bg-green-900/20 dark:text-green-400",
    };

    return (
      <div
        className={`flex items-center gap-2 rounded-lg border px-2 py-1 text-xs font-medium ${probabilityColors[probability]}`}
      >
  ...
  return (
    <div className={`mx-auto w-full max-w-7xl ${className}`}>
      <div className="overflow-hidden rounded-2xl border border-border/50 bg-background">
        <div className="grid grid-cols-7 gap-4 border-b border-border/20 bg-muted/15 px-6 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground/70">
          <div className="flex items-center gap-3">
            <div className="relative">
              <input
                ...
                className="h-4 w-4 rounded border-muted-foreground/40 bg-background text-muted-foreground accent-muted-foreground focus:ring-2 focus:ring-muted-foreground/20"
              />
            </div>
            <span>{title}</span>
          </div>
          ...
          <div>Interest</div>
          <div>Probability</div>
          <div className="flex cursor-pointer items-center gap-2" onClick={handleSort}>
            Last Action
            <ChevronDown
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible">
          {leads.map((lead, index) => (
            <motion.div key={lead.id} variants={rowVariants}>
              <div
                className={`group relative grid cursor-pointer grid-cols-7 gap-4 px-6 py-2 transition-colors hover:bg-muted/30 ${
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex min-w-0 items-center gap-3">
                    <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full border border-border/20 bg-muted/30">
                ...
                <div className="flex items-center">{getSourcePill(lead.source, lead.sourceType)}</div>
                <div className="flex items-center">{getStatusPill(lead.status)}</div>
                <div className="flex items-center">
                  <span className="font-semibold text-foreground/90">{formatCurrency(lead.size)}</span>
                </div>
                <div className="flex items-center">{renderSparkline(lead.interest)}</div>
                <div className="flex items-center">{getProbabilityIcon(lead.probability)}</div>
                <div className="flex items-center">
                  <AnimatePresence mode="wait">
                    {hoveredAction === lead.id ? (
                      <motion.button
                        ...
                        className="flex cursor-pointer items-center gap-2 rounded-lg border border-primary/30 bg-primary/10 px-2 py-1 text-xs font-medium text-primary shadow-sm transition-all duration-200 hover:bg-primary/20 hover:shadow-md"
                      >
                        Engage
                        <div className="mx-1 h-3 w-px bg-primary/30" />
      ...
      <AnimatePresence>
        {selectedLeads.size > 0 ? (
          ...
          >
            <div className="rounded-xl border border-border/50 bg-background/95 px-4 py-2 shadow-2xl backdrop-blur-lg">
