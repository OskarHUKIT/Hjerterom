...
## Steg 4: Database Webhook (valgfritt hvis migrasjon er kjørt)

**Anbefalt:** Kjør migrasjonen `20260607140000_notification_push_trigger.sql` (`supabase db push`). Da kalles `send-push` automatisk via pg_net når et varsel opprettes — du trenger da **ikke** manuell webhook.

**Alternativ (eldre oppsett):** Manuell webhook i Dashboard:
...
**Viktig:** Bruk enten migrasjon-trigger **eller** manuell webhook — ikke begge (ellers doble push-varsler).

### Vault-secrets (påkrevd for migrasjon-trigger)

Kjør én gang i **SQL Editor** hvis ikke allerede satt (samme som handover-cron):

```sql
select vault.create_secret('https://<PROJECT_REF>.supabase.co', 'project_url');
select vault.create_secret('<service_role_key>', 'anon_key');
```
...
- **Ingen varsler?** Sjekk at `20260607140000_notification_push_trigger.sql` er kjørt (eller webhook er aktiv), vault-secrets `project_url` + `anon_key` finnes, og at `send-push`-funksjonen er deployet
- **VAPID-nøkler må matche:** `NEXT_PUBLIC_VAPID_PUBLIC_KEY` (frontend/Vercel) og `VAPID_PUBLIC_KEY` + `VAPID_KEY` (Supabase Secrets) må være samme nøkkelpar
- **«Aktivt» men ingen push?** Gå til Varsler og trykk «Aktiver varsler» på nytt — tidligere versjon viste «aktivt» uten å lagre abonnement
