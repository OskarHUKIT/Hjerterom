...
| 2026-07-04 | 1.2 | §3.6 Turisme booking-melding (NPD-3A): én tråd per booking via `guest_booking` kanal |

---

## 3.6 Turisme booking-melding (NPD-3A)

Én chat-tråd per turisme-booking (Airbnb-konvensjon):

| Aktør | Inngang | RPC / kanal |
|-------|---------|-------------|
| Leietaker | `/finn/mine?booking=<id>` | `get_booking_messages`, `send_booking_message` |
| Utleier | `/nav/messages?booking=<id>` (Leietaker-fane) | Samme RPC |
| Utleier (manage) | Booking-forespørsel → «Melding til leietaker» | Lenke til meldinger |

Datamodell: `chat_messages.channel_type = 'guest_booking'` + `booking_id` FK. Tilgang via `_booking_message_participant()` (SECURITY DEFINER RPC — ikke direkte tabell-SELECT).

Initial melding seedes fra `bookings.message` ved første lesing (`ensure_booking_initial_chat_message`).
